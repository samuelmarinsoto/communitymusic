...
class PagedArray {
public:
    PagedArray(size_t size, size_t objsize, size_t mempages): size_(size), page_count_(size / PAGE_SIZE + (size % PAGE_SIZE != 0)), file_(FILENAME, std::ios::binary | std::ios::in | std::ios::out | std::ios::trunc) {
        if (!file_.is_open()) {
